...
docker run -d --name lazy-aiops \
  ...
  crpi-iihofxt94xlrdrvd.cn-shanghai.personal.cr.aliyuncs.com/lazyops/lazyops:v1.0.42
```
...
IMAGE=crpi-iihofxt94xlrdrvd.cn-shanghai.personal.cr.aliyuncs.com/lazyops/lazyops:v1.0.42
docker pull $IMAGE
...
```bash
# Kubernetes
REGISTRY_IMAGE=registry.example.com/lazy-aiops:v1.0.42 deploy/scripts/deploy.sh k8s

# Docker Swarm
REGISTRY_IMAGE=registry.example.com/lazy-aiops:v1.0.42 deploy/scripts/deploy.sh swarm
```

## 版本信息

- 当前推荐版本：`v1.0.42`（亦支持 `latest`）
- 对应代码提交：请以 `git rev-list -n 1 v1.0.42` 的结果为准
- ACR 镜像示例：
  - `crpi-iihofxt94xlrdrvd.cn-shanghai.personal.cr.aliyuncs.com/lazyops/lazyops:v1.0.42`
