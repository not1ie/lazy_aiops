...
func (s *AuthService) InitDefaultAdmin() error {
	...
	bootstrapPassword, err := resolveBootstrapAdminPassword()
	if err != nil {
		return err
	}

	// 创建默认管理员
	hashedPwd, err := bcrypt.GenerateFromPassword([]byte(bootstrapPassword), bcrypt.DefaultCost)
	if err != nil {
		return err
	}
	admin := User{
	...
	if isAutoGeneratedBootstrapPassword(bootstrapPassword) {
		fmt.Printf("[Auth] 已创建初始管理员账号 admin，临时密码：%s（请登录后立即修改）\n", bootstrapPassword)
	}
	return nil
}
...
func isDefaultAdminPassword(user *User) bool {
	...
	for _, candidate := range defaultAdminPasswordCandidates() {
		if candidate == "" {
			continue
		}
		if bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(candidate)) == nil {
			return true
		}
	}
	return false
...
func resolveBootstrapAdminPassword() (string, error) {
	password := strings.TrimSpace(os.Getenv("LAO_BOOTSTRAP_ADMIN_PASSWORD"))
	if password != "" {
		return password, nil
	}
	if isTruthy(os.Getenv("LAO_ALLOW_INSECURE_BOOTSTRAP")) {
		return "admin123", nil
	}
	random := make([]byte, 24)
	...
	return "AIOps!" + base64.RawURLEncoding.EncodeToString(random), nil
}

func defaultAdminPasswordCandidates() []string {
	candidates := []string{"admin123"}
	if password := strings.TrimSpace(os.Getenv("LAO_BOOTSTRAP_ADMIN_PASSWORD")); password != "" {
		candidates = append(candidates, password)
	}
	return candidates
}

func isAutoGeneratedBootstrapPassword(password string) bool {
	return strings.HasPrefix(password, "AIOps!")
}
