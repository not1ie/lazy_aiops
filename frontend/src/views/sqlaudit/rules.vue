    ...
    <el-row :gutter="12" class="mb-3">
      <el-col :xs="24" :lg="13">
        <el-card shadow="never">
          ...
          <el-table :fit="true" :data="rules" v-loading="loading" stripe>
      ...
      <el-col :xs="24" :lg="11">
        <el-card shadow="never">
          ...
          <el-table :fit="true" v-if="analyzeResult && (analyzeResult.issues?.length || 0) > 0" :data="analyzeResult.issues" size="small" stripe>
            <el-table-column prop="type" label="类型" width="110" />
            <el-table-column prop="level" label="级别" width="90">
              <template #default="{ row }">
                <el-tag :type="levelTagType(row.level)">{{ levelText(row.level) }}</el-tag>
